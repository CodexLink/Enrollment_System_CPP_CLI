...
int main(int argc, char *argv[]) {
    printf("Initializing Mode | Arguments Count is %c", argc);
...
void OldStd_Func_Enrollment_Eng(struct StudentRecords OldStudent_Continuation) {
    ...
    printf("-----------------------------");
    printf("Hello and Welcome %c", OldStudent_Continuation.user_Username);
    Sleep(2000);
    printf("-----------------------------");
...
char CriticalComp_Check() {
    ...
    FILE *FileDatabase;
    char datapoint[] = "data/2018_LM_Database.dat", datapoint_date;
    FileDatabase = fopen(datapoint, "r");
    if (FileDatabase = NULL) {
        printf("Local Database wasn't found~! Creating Local Database to Store Enrollment Data...");
        fopen("data/2018_LM_Database.dat", "w");
        fwrite(datapoint, 1, sizeof(datapoint), FileDatabase);
        fclose(FileDatabase);
        GetLocalTime(&datapoint_date);
        printf("Database Created as of 5c", datapoint_date);
    }
    else {
        fclose(FileDatabase);
